...
export function serializeRecipe(recipe: Recipe) {
  const frontmatter = [
    ...
    `prepTime: ${escapeFrontmatter(recipe.prepTime ?? "")}`,
    `cookTime: ${escapeFrontmatter(recipe.cookTime ?? "")}`,
    `notes: ${escapeFrontmatter(recipe.notes ?? "")}`,
...
export function parseRecipeMarkdown(markdown: string, attachment?: string): Recipe {
  ...
  return {
    ...
    prepTime: parseFrontmatterString(frontmatter, "prepTime") || undefined,
    cookTime: parseFrontmatterString(frontmatter, "cookTime") || undefined,
    notes: parseFrontmatterString(frontmatter, "notes") || undefined,
